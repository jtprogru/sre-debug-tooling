...
`hack/tools.sha256` генерируется, руками его не правят. Пропустишь refresh — сборка упадёт на несовпадении чексуммы и сама скажет, что запустить. В этом и смысл.

Бампы здесь ручные намеренно. Апдейтер знает только про upstream-релизы и утащит `kubectl` на то, что вышло на прошлой неделе, а `kubectl` тут должен следовать за твоим кластером, а не за свежим тегом: допустимый разбег — одна минорная версия в любую сторону. Остальное от отставания на релиз ничего не теряет. То, что двигаться само действительно должно, закрыто [dependabot](.github/dependabot.yml): digest базового образа и версии экшенов в workflow.
